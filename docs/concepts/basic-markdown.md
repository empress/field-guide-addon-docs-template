field-guide uses a language called **Markdown** to format text, just like all the rest of the [Empress products](https://github.com/empress/)

If you want to start a new page we recommend that you use the built in generator `ember generate page "The next big feature - documented with Field Guide"`. This will create a file for you and setup the start of the metadata.

For now, though, let's run you through some of the basics of editing Markdown. You can see this post [directly on github](https://github.com/empress/field-guide/blob/master/blueprints/field-guide/files/__base__/docs/concepts/basic-markdown.md) if you want to see all the Markdown we've used. (don't forget to click the _Raw_ button to see the source).
...
If you want to add images to your field-guide app you can just drop them in the `/public` folder, which you should see if you are running this locally!

_**Important Note:** field-guide  does not currently have automatic image resizing, so it's always a good idea to make sure your images aren't gigantic files **before** adding them to your project._
...
All themes handle blockquotes slightly differently. Sometimes they'll look better kept shorter, while other times you can quote fairly hefty amounts of text and get away with it. Generally, the safest option is to use blockquotes sparingly.
